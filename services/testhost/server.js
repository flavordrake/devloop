#!/usr/bin/env node
/**
 * services/testhost/server.js — Test review + upload server
 *
 * Serves emulator recordings, video frames, test screenshots, reports,
 * and accepts file uploads for mobile review. Server-rendered, no SPA.
 *
 * Configuration via environment variables:
 *   TESTHOST_PORT       — Listen port (default: 9090)
 *   TESTHOST_PROJECT    — Project root to serve artifacts from (default: cwd)
 *   TESTHOST_UPLOAD_DIR — Upload directory (default: $TESTHOST_PROJECT/test-results/uploads)
 *
 * Usage:
 *   TESTHOST_PROJECT=/home/dev/workspace/mobissh node services/testhost/server.js
 */
...
const PORT = Number(process.env.TESTHOST_PORT || process.env.REVIEW_PORT || 9090);

const REPO = process.env.TESTHOST_PROJECT
  ? path.resolve(process.env.TESTHOST_PROJECT)
  : process.cwd();
...
const UPLOAD_DIR = process.env.TESTHOST_UPLOAD_DIR
  ? path.resolve(process.env.TESTHOST_UPLOAD_DIR)
  : path.join(REPO, 'test-results/uploads');
  ...
  return html('Upload', `
    ...
    <div class="card">
      <form method="POST" action="/upload" enctype="multipart/form-data">
        ...
        <label>Files:<br>
          <input type="file" name="file" accept="image/*,video/*" multiple style="margin:0.5rem 0;">
        </label><br>
...
function handleUpload(req, res) {
  ...
  req.on('end', () => {
    ...
    if (!fileParts.length) { res.writeHead(400); res.end('No file'); return; }

    const desc = descPart ? descPart.data.toString('utf8').trim() : '';
    const tsStr = new Date().toISOString().replace(/[:.]/g, '-').substring(0, 19);
    const slug = desc ? slugify(desc) : 'upload';
    const multi = fileParts.length > 1;
    ...
    for (let i = 0; i < fileParts.length; i++) {
      ...
      const filename = `${tsStr}-${prefix}${slug}${ext}`;
      fs.writeFileSync(path.join(UPLOAD_DIR, filename), part.data);
      lastFilename = filename;
    }

    const redirectPath = multi
      ? '/uploads'
      : `/file/test-results/uploads/${lastFilename}`;
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`  Artifacts:  ${ARTIFACT_DIRS.emulator}`);
});
